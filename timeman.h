...
static _FORCE_INLINE utime_t gettime_ns(clockid_t clockid) {
	struct timespec t;
...
static _FORCE_INLINE utime_t gettime_mcs(clockid_t clockid) {
	struct timespec t;
	int c = clock_gettime(clockid, &t);
	ASSERT(c == 0, "Cannot get time with clock_gettime");
...
static _FORCE_INLINE utime_t gettime_mls(clockid_t clockid) {
	struct timespec t;
	int c = clock_gettime(clockid, &t);
	ASSERT(c == 0, "Cannot get time with clock_gettime");
...
static _FORCE_INLINE utime_t gettime_s(clockid_t clockid) {
	struct timespec t;
	int c = clock_gettime(clockid, &t);
	ASSERT(c == 0, "Cannot get time with clock_gettime");
